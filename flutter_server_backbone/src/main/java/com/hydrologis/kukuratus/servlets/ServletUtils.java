...
public class ServletUtils {
    ...
    public static Object canProceed( Request request, Response response, String tag ) throws Exception {
        ...
        if (gpapUser == null) {
            String limitStr = RegistryHandler.INSTANCE.getGlobalSettingByKey(KukuratusSession.KEY_AUTOMATIC_REGISTRATION, "0"); //$NON-NLS-1$
            long limit = Long.parseLong(limitStr);
            long now = System.currentTimeMillis();
            double deltaMinutes = (now - limit) / 60.0 / 1000.0;
            // if (deltaMinutes < KukuratusSession.timerMinutes) {
