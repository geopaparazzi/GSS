...
/**
 * Ormlite type for point spatial objects.
 * 
 * @author Andrea Antonello (www.hydrologis.com)
 */
public class KukuratusPointType extends BaseDataType {
    private static final KukuratusPointType singleTon = new KukuratusPointType();
    private static EDb dbType;
    private static PostgisGeometryParser pg = new PostgisGeometryParser();

    public static KukuratusPointType getSingleton() {
        return singleTon;
    }

    private KukuratusPointType() {
    ...
    private static Class< ? >[] getGeomClass() {
        ...
        case H2GIS:
        default:
            return new Class< ? >[]{org.locationtech.jts.geom.Point.class};
        }
    }
     ...
     */
    protected KukuratusPointType( SqlType sqlType, Class< ? >[] classes ) {
        super(sqlType, classes);
    }

    @Override
    public Object parseDefaultString( FieldType fieldType, String defaultStr ) throws SQLException {
        throw new SQLException("Default values for point types are not supported"); //$NON-NLS-1$
    }
